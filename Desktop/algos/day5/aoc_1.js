...
const solve = async () => {
    ...
    const seatIds = lines.map(line => {
        const rowStr = line.slice(0, 7); //row info, up to 6 not including 7
        const colStr = line.slice(7); //col info, start at idx 7
        const rowNum = (binaryFind(rowStr, 'F', 'B', 128)); //row requirement
        const colNum = (binaryFind(colStr, 'L', 'R', 8)); //col requirement 
        return (8 * rowNum) + colNum;

    });
    return Math.max(...seatIds); //built-in JS method to ge the max value, "... drops the brackets bc arrays do not work on this method"
};
